...
import { useCreateSupplyPurchase } from "@/features/supply-purchases/hooks/use-supply-purchases"
import type { PurchaseStatus, PaymentMethod } from "@/features/supply-purchases/types/supply-purchases.types"
...
export default function CreateSupplyPurchasePage() {
  const router = useRouter()
  const createPurchaseMutation = useCreateSupplyPurchase()
  ...
  const handleUpdateItem = (id: string, field: keyof CartItem, value: any) => {
    setItems((prev) =>
      prev.map((item) => {
        ...
        // If supply changes, auto-fill unitCost with supply's basePrice
        if (field === "supplyId" && suppliesRes?.data) {
          const supply = suppliesRes.data.find((s) => s.id === value)
          if (supply) {
            updated.supplyName = supply.name
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (items.length === 0) {
      toast.error("Debes agregar al menos un insumo para registrar la compra")
      return
    }

    const invalidItem = items.find((item) => !item.supplyId || item.quantity <= 0 || item.unitCost < 0)
    if (invalidItem) {
      toast.error("Por favor completa todos los datos de los insumos agregados")
      return
    }
  ...
  return (
    <div className="space-y-6">
      <title>Registrar Nueva Compra de Insumos | EasyPoint</title>
      
      <div className="flex items-center gap-3">
        <button
          onClick={() => router.push("/supply-purchases")}
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Crea una nueva transacción de compra de insumos para abastecer tus almacenes
          </p>
        </div>
      </div>
          ...
          <div className="bg-card border border-border/40 rounded-2xl shadow-sm overflow-hidden p-5 space-y-4">
            ...
            {items.length === 0 ? (
              <div className="py-12 border border-dashed border-border/20 rounded-xl flex flex-col items-center justify-center text-center space-y-3">
                ...
                <div className="max-w-[280px]">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-1">
                    Agrega insumos a la compra e introduce su cantidad y costo para iniciar la transacción.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <div className="space-y-3">
                ...
                <div className="hidden sm:grid grid-cols-[2fr_120px_130px_100px_40px] gap-3 px-2 text-[10px] font-bold text-muted-foreground uppercase tracking-wider">
                  <span>Insumo</span>
                ...
                <div className="space-y-2">
                  {items.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        className="grid grid-cols-1 sm:grid-cols-[2fr_120px_130px_100px_40px] gap-3 items-center p-3 rounded-xl bg-muted/10 dark:bg-muted/5 border border-border/10 transition-all duration-150 hover:bg-muted/20"
                      >
                        {/* Supply Dropdown */}
                        <div className="space-y-1">
                          ...
                          >
                            ...
                            {loadingSupplies ? (
                              <option disabled>Cargando insumos...</option>
                            ) : (
                              purchasableSupplies.map((sup) => {
                                const supUnit = sup.unitOfMeasure === "GRAM" ? "kg" : sup.unitOfMeasure === "MILLILITER" ? "L" : "und"
                                return (
                                  <option key={sup.id} value={sup.id}>
                                    {sup.name} (Ref: {formatCurrency(sup.basePrice ?? 0)} / {supUnit})
                                  </option>
                                )
                              })
                            )}
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="relative">
                            <input
                              ...
                              placeholder="1"
                              className="w-full h-9 rounded-lg border border-border/40 bg-card px-2.5 pr-8 text-center text-xs font-mono font-semibold text-foreground placeholder:text-muted-foreground/40 focus:border-brand-500/50 outline-none transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            />
                            {unitSuffix && (
                              <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-muted-foreground/60 font-semibold uppercase">
                                {unitSuffix}
                              </span>
                            )}
                          </div>
                ...
                <div className="text-[10px] text-muted-foreground/80 leading-normal p-2.5 bg-brand-500/5 border border-brand-500/10 rounded-xl">
                  💡 **Nota sobre Unidades**: Las cantidades de peso (g) y volumen (ml) deben ingresarse en su unidad de referencia mayor (Kilogramos o Litros). Por ejemplo, ingresa `0.3` para registrar `300 g` o `1.5` para registrar `1500 ml`.
                </div>
              </div>
            )}
          </div>
