...
import {
  ...
  useCompleteProductPurchase,
  useAddProductPurchaseItems,
} from "@/features/product-purchases/hooks/use-product-purchases"
...
} from "@/features/product-purchases/types/product-purchases.types"
import {
  ShoppingCart,
  Trash2,
  Eye,
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          <div>
            <h4 className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1 mb-3">
              <Package className="h-3 w-3" /> Ítems comprados
            </h4>
            ...
            {isLoadingMovements ? (
              ...
              <div className="h-20 border border-dashed border-border/25 rounded-xl flex items-center justify-center text-xs text-muted-foreground/60 gap-2">
                ...
                <span>No hay ítems registrados en esta compra</span>
              </div>
            )}
          </div>
...
interface CompletePurchaseModalProps {
  ...
  onClose: () => void
  purchase: ProductPurchase | null
  onConfirm: (payload: CompleteProductPurchaseDTO) => void
  isLoading: boolean
}
...
function CompletePurchaseModal({ isOpen, onClose, purchase, onConfirm, isLoading }: CompletePurchaseModalProps) {
  ...
  function handleSubmit() {
    ...
    const payload: CompleteProductPurchaseDTO = {
      bankAccountId,
      paymentMethod,
...
function AddPurchaseItemsModal({ isOpen, onClose, purchase, onConfirm, isLoading }: AddPurchaseItemsModalProps) {
  const [items, setItems] = React.useState<CartItemInput[]>([])
  const [initialized, setInitialized] = React.useState(false)
  
  // Fetch active products with isPurchased = true
  ...
  React.useEffect(() => {
    if (isOpen && purchase && !initialized && !isLoadingMovements) {
      if (movementsResponse?.data && movementsResponse.data.length > 0) {
        ...
        setItems([{ id: Math.random().toString(36).substring(7), productId: "", quantity: 1, unitCost: 0 }])
      }
      setInitialized(true)
    }
  ...
  const handleAddItem = () => {
    setItems((prev) => [...prev, { id: Math.random().toString(36).substring(7), productId: "", quantity: 1, unitCost: 0 }])
  }
  ...
  function handleSubmit() {
    ...
    const invalid = items.find((i) => !i.productId || i.quantity <= 0 || i.unitCost < 0)
    if (invalid) {
      ...
      <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-lg rounded-2xl bg-card border border-border/40 shadow-xl p-5 sm:p-6 gap-5 duration-200 max-h-[85vh] overflow-y-auto">
        <DialogHeader className="pb-3 border-b border-border/10">
          <DialogTitle className="text-base font-heading font-bold text-foreground">
            Actualizar productos de Compra #{purchase.id.slice(-8).toUpperCase()}
          </DialogTitle>
          <p className="text-xs text-muted-foreground mt-1">
            Esta compra está pendiente. Puedes agregar, modificar o eliminar productos de la compra actual. Los cambios afectarán el inventario físico.
          </p>
        </DialogHeader>
        ...
        {isLoadingMovements ? (
          <div className="h-40 flex flex-col items-center justify-center text-xs text-muted-foreground/60 gap-2">
            ...
            <span>Cargando productos de la compra...</span>
          </div>
        ) : (
          <div className="space-y-4 my-2">
            {items.map((item) => (
              <div key={item.id} className="grid grid-cols-1 sm:grid-cols-[2fr_80px_100px_40px] gap-2 items-center p-2 rounded-xl bg-muted/20 border border-border/10">
                <select
                ...
                <input
                  ...
                  placeholder="Cant"
                  className="w-full h-8 rounded-lg border border-border/30 bg-card text-center text-xs text-foreground outline-none focus:border-brand-500/50"
                />
...
export default function ProductPurchasesPage() {
  ...
  // ─── Mutations ───────────────────────────────────────────────────────────
  const deletePurchaseMutation = useDeleteProductPurchase()
  const completePurchaseMutation = useCompleteProductPurchase()
  const addItemsMutation = useAddProductPurchaseItems()
  ...
  const [selectedPurchase, setSelectedPurchase] = React.useState<ProductPurchase | null>(null)
  ...
  // ─── Query ───────────────────────────────────────────────────────────────
  const queryParams: FindProductPurchasesParams = {
    page,
  ...
  const { data: response, isLoading, error } = useProductPurchases(queryParams)
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return name ? (
          ...
          <span className="text-xs text-muted-foreground/50 italic">Sin registrar</span>
        )
      },
    },
    {
  ...
  function handleComplete(payload: CompleteProductPurchaseDTO) {
    if (!selectedPurchase) return
  ...
  function handleAddItems(payload: AddItemsProductPurchaseDTO) {
    ...
    addItemsMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Productos agregados con éxito a la compra")
          setIsAddItemsOpen(false)
          setIsDetailOpen(false)
          setSelectedPurchase(null)
        },
        onError: () => toast.error("Error al agregar productos a la compra"),
      }
    )
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTableToolbar
        ...
        actionSection={
          <DataTableAction
            ...
            shape="md"
            onClick={() => router.push("/product-purchases/create")}
          />
        }
      />
      ...
      <DataTable
        ...
        data={response?.data || []}
        loading={isLoading}
        emptyMessage="No se encontraron compras de productos. Ajusta los filtros o registra una nueva compra."
        sortKey={sortKey}
        sortOrder={sortOrder}
        onSort={handleSort}
        pagination={{
      ...
      <ConfirmModal
        ...
        description={
          selectedPurchase?.status === "COMPLETED"
            ? `Esta compra ya está completada. Si continúas, se revertirá el inventario incrementado, se reembolsará el dinero a la cuenta bancaria original de la transacción y se marcará la compra como CANCELADA. ¿Deseas continuar?`
            : `Esta compra está pendiente. Se revertirá el stock de los productos y se eliminará permanentemente la compra. ¿Deseas continuar?`
        }
