...
import {
  ...
  useToggleTransactionCategoryActive,
} from "@/features/transaction-categories/hooks/use-transaction-categories"
import type {
  TransactionCategory,
  FindTransactionCategoriesParams,
} from "@/features/transaction-categories/types/transaction-categories.types"
import {
  Pencil,
  Trash2,
  FolderOpen,
...
// ─── Category Card ─────────────────────────────────────────────────────────────
// Horizontal pill-style card: [type] [name + status ···] [⏻ | ✏ 🗑]
...
function CategoryCard({
  ...
  return (
    ...
    >
      ...
      <span
        className={cn(
          "inline-flex items-center justify-center shrink-0 h-7 w-12 rounded-lg",
          "text-[9px] font-black tracking-wider border select-none",
          isIncome
      ...
      <div className="text-center space-y-1">
        ...
        <p className="text-xs text-muted-foreground max-w-[220px]">
          {isFiltered
            ...
            : "Aún no hay categorías de transacción registradas. Crea la primera."}
        </p>
      </div>
      {isFiltered && (
        <button
...
export default function TransactionCategoriesPage() {
  ...
  const toggleMutation = useToggleTransactionCategoryActive()

  // ─── Modal States ──────────────────────────────────────────────────────────
  const [isCreateOpen, setIsCreateOpen] = React.useState(false)
  const [isEditOpen, setIsEditOpen] = React.useState(false)
  const [isDeleteOpen, setIsDeleteOpen] = React.useState(false)
  const [selectedRecord, setSelectedRecord] = React.useState<TransactionCategory | null>(null)
  ...
  // ─── Query ─────────────────────────────────────────────────────────────────
  const queryParams: FindTransactionCategoriesParams = {
    page,
    limit,
  ...
  const { data: response, isLoading } = useTransactionCategories(queryParams)
  ...
  function handleDelete() {
    ...
    deleteMutation.mutate(selectedRecord.id, {
      ...
      onError: (err: any) => {
        const msg =
          err?.response?.data?.message ??
          "Error al eliminar la categoría. Asegúrese de que no contenga transacciones."
        toast.error(msg)
      },
    })
  }

  function handleToggle(category: TransactionCategory) {
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTableToolbar
        ...
        actionSection={
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
            <MasterImportActions resource="transaction-categories" />
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
          {Array.from({ length: 10 }).map((_, i) => (
            ...
            >
              <div className="h-7 w-12 rounded-lg bg-muted/60 shrink-0" />
      ...
      <DynamicFormModal
        ...
        title="Nueva Categoría de Transacción"
        fields={fields}
      ...
      <DynamicFormModal
        ...
        title="Editar Categoría de Transacción"
        fields={fields}
      ...
      <ConfirmModal
        ...
        description={
          selectedRecord
            ? `Estás a punto de eliminar "${selectedRecord.name}". Esta acción no se puede deshacer y fallará si hay transacciones asociadas.`
