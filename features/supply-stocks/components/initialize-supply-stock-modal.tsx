...
import { useCreateSupplyStock } from "../hooks/use-supply-stocks"
...
interface InitializeSupplyStockModalProps {
  isOpen: boolean
  onClose: () => void
  existingSupplyIds: string[]
}

export function InitializeSupplyStockModal({
  isOpen,
  onClose,
  existingSupplyIds,
}: InitializeSupplyStockModalProps) {
  const createStockMutation = useCreateSupplyStock()
  ...
  // Fetch all active supplies (limit 100)
  const { data: suppliesResponse, isLoading: isSuppliesLoading } = useSupplies({ limit: 100, isActive: true })
  const supplies = suppliesResponse?.data || []

  // Filter out supplies that already have a stock record
  const availableSupplies = React.useMemo(() => {
    return supplies.filter((s) => !existingSupplyIds.includes(s.id))
  }, [supplies, existingSupplyIds])
  ...
  const validate = () => {
    ...
    if (!supplyId) {
      newErrors.supplyId = "El insumo es obligatorio"
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createStockMutation.mutate(
      ...
      {
        onSuccess: () => {
          toast.success("Stock de insumo inicializado con éxito")
          onClose()
        },
        onError: (err) => {
      ...
      <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-md rounded-xl bg-card border border-border/40 shadow-xl p-6 gap-6">
        <DialogHeader className="gap-1">
          <DialogTitle className="text-xl font-heading font-semibold text-foreground">
            Inicializar Stock de Insumo
          </DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Crea un registro de stock con cantidad inicial 0 para insumos con datos deprecados.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="supply-stock-supply" className="text-xs font-bold text-muted-foreground/90">
              ...
              {isSuppliesLoading ? (
                ...
                <div className="text-sm text-amber-600 dark:text-amber-400 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
                  No hay insumos activos sin stock configurado actualmente.
                </div>
              ) : (
                <>
                  <Select value={supplyId} onValueChange={(val) => {
                    setSupplyId(val || "")
                    if (errors.supplyId) setErrors((prev) => ({ ...prev, supplyId: "" }))
                  }}>
                    <SelectTrigger
                      id="supply-stock-supply"
                  ...
                  {errors.supplyId && (
                    <span className="text-xs text-destructive mt-0.5">{errors.supplyId}</span>
                  )}
                </>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="supply-stock-location" className="text-xs font-bold text-muted-foreground/90">
                Ubicación <span className="text-destructive font-bold">*</span>
              </Label>
              <Input
                id="supply-stock-location"
            ...
            {/* Min Quantity */}
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="supply-stock-min" className="text-xs font-bold text-muted-foreground/90">
          ...
          <DialogFooter className="gap-2 sm:gap-0 mt-6 border-t border-border/40 pt-4 flex flex-row items-center justify-end">
            ...
            <Button
              type="submit"
              disabled={createStockMutation.isPending || !supplyId || availableSupplies.length === 0}
