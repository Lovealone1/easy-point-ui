...
import { useUpdateProductStock } from "../hooks/use-product-stocks"
...
interface EditMinQuantityModalProps {
  isOpen: boolean
  onClose: () => void
  stock: ProductStock | null
}

export function EditMinQuantityModal({
  isOpen,
  onClose,
  stock,
}: EditMinQuantityModalProps) {
  const updateStockMutation = useUpdateProductStock()
  const { data: product } = useProduct(stock?.productId || "")
      ...
      <DialogContent className="max-w-[calc(100%-2rem)] sm:max-w-md rounded-xl bg-card border border-border/40 shadow-xl p-6 gap-6">
        <DialogHeader className="gap-1">
          ...
          <DialogDescription className="text-sm text-muted-foreground">
            {product ? `Modifica la alerta de stock mínimo para: ${product.name}` : "Modifica el valor límite para alertas de stock."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-4">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="edit-stock-min" className="text-xs font-bold text-muted-foreground/90">
