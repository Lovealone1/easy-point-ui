...
export interface FindSupplyPurchasesParams {
  page?: number;
  limit?: number;
  order?: 'ASC' | 'DESC';
  orderBy?: string;
...
export interface CreateSupplyPurchaseDTO {
  supplierId?: string;
  status?: PurchaseStatus;
...
export interface CompleteSupplyPurchaseDTO {
  bankAccountId: string;
  paymentMethod?: PaymentMethod;
  categoryId?: string;
  notes?: string;
...
export interface AddItemsSupplyPurchaseDTO {
  items: SupplyPurchaseItem[];
}
