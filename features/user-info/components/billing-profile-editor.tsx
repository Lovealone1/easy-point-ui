// ─────────────────────────────────────────────────────────────────────────────
// features/user-info/components/billing-profile-editor.tsx
//
// The DIAN electronic-invoicing profile editor: the active-configuration card
// when one exists, and the two-tab natural/legal person form when it does not.
//
// Extracted verbatim from app/(admin)/admin/user-info/[userId]/page.tsx when
// the same editor was needed in account settings. It is ~700 lines of fiscal
// form — a second copy would be a second place to forget a DIAN field, and
// both copies write to the same two database tables.
//
// The two call sites differ only in whose profile they load and where the
// mutations point, so those arrive as props: the administration console passes
// hooks bound to a userId, account settings passes the /me ones.
// ─────────────────────────────────────────────────────────────────────────────
...
import type { BillingProfile, PersonaNaturalBilling, PersonaJuridicaBilling } from "../types/user-info.types"
...
/** Just enough of a React Query mutation for this component to drive it. */
interface EditorMutation<TPayload> {
...
interface BillingProfileEditorProps {
  ...
  /**
   * Who the profile belongs to, dropped into the sentence "se vincula
   * automáticamente en las transacciones de …". The console passes a name,
   * account settings passes "tu cuenta".
   */
  subjectLabel: string
  configureNatural: EditorMutation<Record<string, unknown>>
  configureJuridica: EditorMutation<Record<string, unknown>>
  deleteProfile: { mutateAsync: () => Promise<unknown>; isPending: boolean }
  /** Omitted in account settings, which has no "back" to offer. */
  onCancel?: () => void
}
...
export function BillingProfileEditor({
  ...
  // The resolution/numbering block is identical on both profiles, so it is
  // rendered once off whichever one is present.
  const sharedData = profile?.data as
    | Pick<
        PersonaNaturalBilling,
        'resolucionDian' | 'prefijoNumeracion' | 'rangoDesde' | 'rangoHasta' | 'modoOperacion' | 'fechaResolucion' | 'fechaVigencia'
      >
    | undefined;
  ...
  const handleNatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(apiErrorMessage(err, "Error al configurar el perfil de facturación"))
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      toast.error(apiErrorMessage(err, "Error al eliminar el perfil"))
    }
  }
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Esta información se vincula automáticamente en las transacciones de {subjectLabel}.
                    </p>
                  </div>
                </div>
                        ...
                        {FISCAL_RESPONSIBILITIES.map((r) => (
                          ...
                          >
                            <input
                              ...
                              onChange={() => handleNatRespToggle(r.code)}
                        ...
                        {FISCAL_RESPONSIBILITIES_JURIDICA.map((r) => (
                          ...
                          >
                            <input
                              ...
                              onChange={() => handleJurRespToggle(r.code)}
                              className="rounded border-border text-primary focus:ring-primary h-3.5 w-3.5"
                            />
                            <span>{r.label}</span>
                          </label>
